...
function transport(t: TestContext) {
  ...
  const fetchMock = t.mock.method(globalThis, 'fetch', async () => Response.json(snapshot(8)));
...
test('HTTP client sends configured project and encoded session controls without caching', async t => {
  ...
  t.mock.method(globalThis, 'fetch', async (url: string | URL | Request, init?: RequestInit) => { calls.push({ url: String(url), init }); return calls.length === 1 ? Response.json(snapshot()) : new Response(null, { status: 204 }); });
  ...
  assert.deepEqual(JSON.parse(calls[0].init?.body as string), { projectConfigId: 'configured-project', mode: 'existing_repo' });
...
test('HTTP errors and invalid snapshots are surfaced instead of reporting success', async t => {
  ...
  fetchMock.mock.mockImplementation(async () => Response.json({ id: 'session-a' }));
  await assert.rejects(createHttpClient().snapshot('session-a'));
  fetchMock.mock.mockImplementation(async () => Response.json({ ...snapshot(), id: 'another-session' }));
...
test('unsubscribe during snapshot fetch prevents delivery and stream recreation', async t => {
  ...
  t.mock.timers.tick(1000);
  unsubscribe();
  resolveFetch(Response.json(snapshot(8)));
  await flush();
  assert.deepEqual(received, []);
  assert.equal(FakeEventSource.instances.length, 1);
});
...
test('reconnect rejects stale snapshots and retries without moving sequence backward', async t => {
  ...
  fetchMock.mock.mockImplementation(async () => Response.json(snapshot(3)));
  source.send(event(5));
  source.onerror?.();
  t.mock.timers.tick(1000);
  await flush();
  assert.deepEqual(received.map(e => e.sequence), [5]);
  assert.equal(FakeEventSource.instances.length, 1);
  fetchMock.mock.mockImplementation(async () => Response.json(snapshot(8)));
